...
export function MatrixInput({ matrix, onChange, errors, readonly, isCompleted }: MatrixInputProps) {
  ...
  const handleBlur = (i: number, j: number) => {
    ...
    if (newNumericMatrix[i][j] !== numericValue) { // Only update if value actually changed
        newNumericMatrix[i][j] = numericValue;
        onChange(newNumericMatrix);
    } else if (numericValue === 0 && currentDisplayValue !== "0" && currentDisplayValue !== "") {
  ...
  return (
    ...
    >
      ...
      <div className={`px-2 py-1 rounded-md ${isDark ? 'border-l-2 border-r-2 border-slate-600' : 'border-l-2 border-r-2 border-slate-400'}`}>
        <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${matrix[0]?.length || 1}, minmax(0, 1fr))` }}>
          {matrix.map((row, i) =>
            row.map((_, j) => ( // Iterate using matrix for structure, but use displayValues for value
              <Input
                key={`${i}-${j}`}
                type="text"
                ...
                className={`w-16 h-12 text-center text-sm font-mono transition-colors duration-150 rounded-md ${
                  readonly || isCompleted
                    ? (isDark ? 'border-green-500 bg-green-700/20 text-green-300 focus-visible:ring-green-500/50' : 'border-green-400 bg-green-100 text-green-700 focus-visible:ring-green-400/50')
                    : errors?.[i]?.[j]
                      ? (isDark ? 'border-red-500 bg-red-700/20 text-red-300 focus-visible:ring-red-500/50' : 'border-red-400 bg-red-100 text-red-700 focus-visible:ring-red-400/50')
                      : (isDark ? 'bg-slate-700 border-slate-600 text-slate-200 focus-visible:ring-slate-500/50' : 'bg-slate-50 border-slate-300 text-slate-800 focus-visible:ring-slate-400/50')
                }`}
